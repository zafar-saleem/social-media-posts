import Image from "next/image";
import styles from "./reaction.module.css";

const icons = {
  like: "data:image/svg+xml;base64,",
  love: "data:image/svg+xml;base64,",
  insight: "data:image/svg+xml;base64,",
  support: "data:image/svg+xml;base64,",
}

export const Reaction = () => {
  return (
    <>
      <button popoverTarget="reaction_popover" className={`${styles.btn_like}`}>
        <Image 
          src={icons.like}
          width={24}
          height={24}
          alt=""
        />
      </button>
      <div id="reaction_popover" popover="auto" className={styles.reaction_popover}>
        <button title="Like" className={`${styles.btn_reaction} ${styles.btn} ${styles.btn_like_icon}`}>
          <Image 
            src={icons.like}
            width={24}
            height={24}
            alt=""
          />
        </button>
        <button title="Awesome" className={`${styles.btn_reaction} ${styles.btn} ${styles.btn_love_icon}`}>
          <Image 
            src={icons.love}
            width={24}
            height={24}
            alt=""
          />
        </button>
        <button title="Insightful" className={`${styles.btn_reaction} ${styles.btn} ${styles.btn_insight_icon}`}>
          <Image 
            src={icons.insight}
            width={24}
            height={24}
            alt=""
          />
        </button>
        <button title="Support" className={`${styles.btn_reaction} ${styles.btn} ${styles.btn_support_icon}`}>
          <Image 
            src={icons.support}
            width={24}
            height={24}
            alt=""
          />
        </button>
      </div>
    </>
  )
}