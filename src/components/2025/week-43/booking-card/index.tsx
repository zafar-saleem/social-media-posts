import Image from "next/image";
import styles from "./booking.module.css";
import { headers } from "next/headers";
import React from "react";

const BookingData = {
  title: "Palm Serenity",
  rate: "Top rated",
  description: "Tropical villa surrounded by rice fields, featuring an open living space and infinity pool.",
  tags: ["Bali", "Indonesia"],
  prices: "$319 / night",
  features_list: ["Pool", "Privte beach"],
  reviews: "4.9",
  images: [
    {
      src: "https://images.pexels.com/photos/13914273/pexels-photo-13914273.jpeg",
      width: 100,
      height: 100,
      alt: "Palm Serenity",
    },
    {
      src: "https://images.pexels.com/photos/6129984/pexels-photo-6129984.jpeg",
      width: 100,
      height: 100,
      alt: "Palm Serenity",
    },
    {
      src: "https://images.pexels.com/photos/11669578/pexels-photo-11669578.jpeg",
      width: 100,
      height: 100,
      alt: "Palm Serenity",
    },
  ]
}

export const BookingCard = () => {
  return (
    <section className={styles.card}>
      <section className={styles.header}>
        <h1 className={styles.title}>{BookingData.title}</h1>
        <span className={styles.badge}>{BookingData.rate}</span>
      </section>
      <p className={styles.description}>{BookingData.description}</p>
      <section className={styles.tags}>
        {
          BookingData.tags.map((tag, index) => (
            <React.Fragment key={index}>
              <span className={styles.tag}>{tag}</span>
            </React.Fragment>
          ))
        }
      </section>
      <section className={styles.meta}>
        <span>{BookingData.prices}</span>
        <button className={styles.button}>
          <span>Book Now</span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-circle-arrow-out-up-right-icon lucide-circle-arrow-out-up-right"><path d="M22 12A10 10 0 1 1 12 2"/><path d="M22 2 12 12"/><path d="M16 2h6v6"/></svg>
        </button>
      </section>
      <section className={styles.carousel}>
        {
          BookingData.images.map((image, index) => (
            <figure className={styles.media} key={index}>
              <Image
                src={image.src}
                width={image.width}
                height={image.height}
                alt={image.alt}
              />
              <figcaption>
                <section className={styles.inclusions}>
                  {
                    BookingData.features_list.map((feature, i) => (
                      <span key={i}>{feature}</span>
                    ))
                  }
                </section>
                <span className={styles.ratings}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#ffffff" stroke="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-star-icon lucide-star"><path d="M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.123 2.123 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.123 2.123 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.122 2.122 0 0 0-1.973 0L6.396 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.122 2.122 0 0 0-.611-1.879L2.16 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.122 2.122 0 0 0 1.597-1.16z"/></svg>
                  {BookingData.reviews}
                </span>
              </figcaption>
            </figure>
          ))
        }
      </section>
    </section>
  )
}